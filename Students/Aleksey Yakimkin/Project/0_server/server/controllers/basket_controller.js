...
module.exports = {
    async load(req, res) {
        let newBasketCollection;
        try {
            newBasketCollection = await Basket.find({ user_id: req.params.id});

            if (!newBasketCollection.length) {
                newBasketCollection.push(await Basket.create({
            ...
            res.json(newBasketCollection[0])
        }
    },
    async add(req, res) {
        try {
